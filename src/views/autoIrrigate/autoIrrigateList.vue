...
const onSubmit = (values: any) => {
  ...
  if (controlStatus.value === 'on') {
    ...
    SendControlCommand({ ...payload, Command: 1 }).then((res) => {
      if ((res as any).IsSuccess) {
        handleCloseDialog()
        closeToast();
      }
    });
  } else if (controlStatus.value === 'off') {
    showLoadingToast({
      message: '发送命令中...',
      forbidClick: true,
      loadingType: 'spinner',
    ...
    SendControlCommand({ ...payload, Command: 0 }).then((res) => {
      if ((res as any).IsSuccess) {
        handleCloseDialog()
        closeToast();
      }
