...
import { getHistoryDataList } from '@/api/field';
...
// 获取历史曲线基本信息
const getLineBaseInfo = async (ObjId: string, item: DateItem) => {
  showLoadingToast({
    message: 'loading...',
    forbidClick: true,
    loadingType: 'spinner',
  ...
  const payload = {
    ...
    ObjId,
    Uid: store.userInfo.user.iotUser
  };
  loading.value = true;
  const res: any = await getHistoryDataList(payload);
...
const onRefresh = () => {
  ...
  getLineBaseInfo(countObjId.value, rangeCalendar);
...
onMounted(async () => {
  ...
  // 获取基本信息
  await getLineBaseInfo(countObjId.value, rangeCalendar);
});
</script>
