<template>
  <div :style="videoWrapStyles">
    <video v-if="props.src" id="videoMain" ref="videoRef"
...
const initVideo = () => {
  nextTick(() => {
    ...
    if (videoRef.value) {
      // 创建 video 实例
      player.value = videojs(videoRef.value, videojs.obj.merge(options));   

      // 创建 video 实例
      // player.value = videojs(videoRef.value, videojs.obj.mergeOptions(options));
