<template>
  <div class="cellVoltage">
    <div class="head">
      <div class="head-left">
        <img src="@/assets/cellVoltage.svg" alt="" />
        <span>电池电压</span>
      </div>
      <div class="head-right">
        <span>最新:</span>
        <span>{{countNewestVoltage}}V</span>
      </div>
    </div>
    <div id="cellVoltageMain" style="width: 100%; height: 250px"></div>
  </div>
</template>
...
// 计算最新电池电压
const countNewestVoltage: ComputedRef = computed(() => {
  const newestNum = electricList.pestList.slice(-1)[0].CellVoltage;
  return newestNum.toFixed(2);
});
// chart实列
const myChart: any = ref(null);
// 绘制图表
const initChart = () => {
  const main = document.getElementById('cellVoltageMain') as HTMLDivElement;
  ...
  const ParamValList = pestList.map((item) => item.CellVoltage);
      ...
      axisPointer: {
        ...
        label: {
          ...
          formatter(params: any) {
            const paramStr = pestList.filter((item) => item.Ctime === params.value)[0].CellVoltage;
            return `电池电压\n${paramStr}V\n${params.value}`;
          },
        },
        lineStyle: {
