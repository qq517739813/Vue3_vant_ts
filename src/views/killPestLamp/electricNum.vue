<template>
  <div class="electricNum">
    <div class="head">
      <div class="head-left">
        <img src="@/assets/electricNum.svg" alt="" />
        <span>电击次数</span>
      </div>
      <div class="head-right">
        <span>最新:</span>
        <span>{{ countNewestNum }}次</span>
      </div>
    </div>
    <div id="electricMain" style="width: 100%; height: 250px"></div>
  </div>
</template>
...
// 计算最新电击次数
const countNewestNum: ComputedRef = computed(() => {
  const newestNum = electricList.pestList.slice(-1)[0].Num;
  return newestNum;
});
// chart实列
...
// 绘制图表
const initChart = () => {
  const main = document.getElementById('electricMain') as HTMLDivElement;
  ...
  const ParamValList = pestList.map((item) => item.Num);
      ...
      axisPointer: {
        ...
        label: {
          ...
          formatter(params: any) {
            const paramStr = pestList.filter((item) => item.Ctime === params.value)[0].Num;
            return `电击次数\n${paramStr}次\n${params.value}`;
          },
        },
        lineStyle: {
