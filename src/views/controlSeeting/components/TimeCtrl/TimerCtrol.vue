<template>
    <div class="timerCtrolCard" v-for="item in props.TimerCtrolListres" :key="item.Id">
        <div class="timeCtroltitle">
            <div class="titletime">{{ item.Optime }}</div>
            <div class="titleBtn">
        ...
        <div class="timeCtrovalue">
            <div class="valueTop">
                <van-row >
                    <van-col span="8">
                        是否定期：{{ item.IsDate === true ? "永久" : "定期" }}
                    </van-col>
                    <van-col span="8">
                        控制动作：{{ item.Opaction === 0 ? "关" : "开" }}
                    </van-col>
                    <van-col span="8">
                        启用状态：{{ item.Enabled === true ? "启用" : "禁用" }}
                    </van-col>
                </van-row>
            </div>
            <div class="valueTime">启止日期：{{ `${item.Bdate?.split(' ')[0]}~${item.Edate?.split(' ')[0]} ` }}</div>
        </div>
    </div>
</template>
...
import { DelTimerCtrl } from "@/api/autoControl"
import { showLoadingToast, closeToast, showToast } from 'vant';
import { userStore } from '@/store/user';

interface Props {
    TimerCtrolListres: CommonItem[];
}
...
const delTimeCtrol = (Id: any) => {
    ...
    DelTimerCtrl(timeCtrlinfo).then(() => {
        emit('getCtrlinfo')
        closeToast();
        showToast("删除成功")
    })
